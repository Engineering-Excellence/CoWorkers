        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            ...
            <div class="table-responsive">
                ...
                <table class="table" style="margin-left: auto; margin-right: auto;">
                    <tr class="bg-info">
                        <th colspan="10" style="font-size: 30px; text-align: center;">중요쪽지함</th>
                    </tr>
                    ...
                    <tbody>
                    <c:set var="list" value="${memoImportantList.list}"/>
                    ...
                    <c:if test="${list.size() != 0}">
                        <c:forEach var="dto" items="${list}">
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <a href="memoView?memoID=${dto.memoID}">
                    ...
                    <td colspan="9" align="center">
                        ...
                        <c:if test="${memoImportantList.currentPage>1}">
                        ...
                        <c:if test="${memoImportantList.startPage>1}">
                            <input type="button" class="button" value="◀◀" title="이전 10 페이지로"
                                   onclick="location.href='?currentPage=${memoImportantList.currentPage-10}'">
                        </c:if>
                        <c:if test="${memoImportantList.startPage<=1}">
                        ...
                        <c:if test="${memoImportantList.currentPage>1}">
                            <input type="button" class="button" value="◀" title="이전 페이지로"
                                   onclick="location.href='?currentPage=${memoImportantList.currentPage-1}'">
                        </c:if>
                        <c:if test="${memoImportantList.currentPage<=1}">
                            <input type="button" class="button2" disabled value="◀" title="현재 첫 페이지 입니다.">
                        </c:if>
                        ...
                        <c:forEach var="i" begin="${memoImportantList.startPage}" end="${memoImportantList.endPage}" step="1">
                            <c:if test="${i==memoImportantList.currentPage}">
                                <ruby>
                                    <b>[${i}]</b>
                                    <rt>▼</rt>
                                </ruby>
                        ...
                        <c:if test="${memoImportantList.currentPage<memoImportantList.totalPage}">
                        ...
                        <c:if test="${memoImportantList.endPage<memoImportantList.totalPage}">
                            <input type="button" class="button" value="▶▶" title="다음 10페이지로"
                                   onclick="location.href='?currentPage=${memoImportantList.startPage+10}'">
                        </c:if>
                        <c:if test="${memoImportantList.endPage>=memoImportantList.totalPage}">
                        ...
                        <c:if test="${memoImportantList.currentPage<memoImportantList.totalPage}">
                            <input type="button" class="button" value="▶▶▶" title="마지막 페이지로"
                                   onclick="location.href='?currentPage=${memoImportantList.totalPage}'">
                        </c:if>
                        <c:if test="${memoImportantList.currentPage>=memoImportantList.totalPage}">
                            <input type="button" class="button2" disabled value="▶▶▶" title="현재 마지막 페이지 입니다.">
                        </c:if>
