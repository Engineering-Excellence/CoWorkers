        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            ...
            <div class="table-responsive">
                ...
                <table class="table" style="margin-left: auto; margin-right: auto;">
                    <tr class="bg-info">
                        <th colspan="10" style="font-size: 30px; text-align: center;">신규쪽지함</th>
                    </tr>
                    ...
                    <tbody>
                    <c:set var="list" value="${memoNewList.list}"/>
                    ...
                    <c:if test="${list.size() != 0}">
                        <c:forEach var="dto" items="${list}">
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <a href="newView?memoID=${dto.memoID}">
                    ...
                    <td colspan="9" align="center">
                        ...
                        <c:if test="${memoNewList.currentPage>1}">
                        ...
                        <c:if test="${memoNewList.startPage>1}">
                            <input type="button" class="button" value="◀◀" title="이전 10 페이지로"
                                   onclick="location.href='?currentPage=${memoNewList.currentPage-10}'">
                        </c:if>
                        <c:if test="${memoNewList.startPage<=1}">
                        ...
                        <c:if test="${memoNewList.currentPage>1}">
                            <input type="button" class="button" value="◀" title="이전 페이지로"
                                   onclick="location.href='?currentPage=${memoNewList.currentPage-1}'">
                        </c:if>
                        <c:if test="${memoNewList.currentPage<=1}">
                            <input type="button" class="button2" disabled value="◀" title="현재 첫 페이지 입니다.">
                        </c:if>
                        ...
                        <c:forEach var="i" begin="${memoNewList.startPage}" end="${memoNewList.endPage}" step="1">
                            <c:if test="${i==memoNewList.currentPage}">
                                <ruby>
                                    <b>[${i}]</b>
                                    <rt>▼</rt>
                                </ruby>
                        ...
                        <c:if test="${memoNewList.currentPage<memoNewList.totalPage}">
                        ...
                        <c:if test="${memoNewList.endPage<memoNewList.totalPage}">
                            <input type="button" class="button" value="▶▶" title="다음 10페이지로"
                                   onclick="location.href='?currentPage=${memoNewList.startPage+10}'">
                        </c:if>
                        <c:if test="${memoNewList.endPage>=memoNewList.totalPage}">
                        ...
                        <c:if test="${memoNewList.currentPage<memoNewList.totalPage}">
                            <input type="button" class="button" value="▶▶▶" title="마지막 페이지로"
                                   onclick="location.href='?currentPage=${memoNewList.totalPage}'">
                        </c:if>
                        <c:if test="${memoNewList.currentPage>=memoNewList.totalPage}">
                            <input type="button" class="button2" disabled value="▶▶▶" title="현재 마지막 페이지 입니다.">
                        </c:if>
