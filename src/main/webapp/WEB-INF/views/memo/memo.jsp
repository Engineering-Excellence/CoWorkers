        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            ...
            <div class="table-responsive">
                ...
                <table class="table" style="margin-left: auto; margin-right: auto;">
                    <tr class="bg-info">
                        <th colspan="10" style="font-size: 30px; text-align: center;">쪽지함</th>
                    </tr>
                    ...
                    <tbody>
                    <c:set var="list" value="${memoList.list}"/>
                    ...
                    <td colspan="9" align="center">
                        ...
                        <c:if test="${memoList.currentPage>1}">
                        ...
                        <c:if test="${memoList.startPage>1}">
                            <input type="button" class="button" value="◀◀" title="이전 10 페이지로"
                                   onclick="location.href='?currentPage=${memoList.currentPage-10}'">
                        </c:if>
                        <c:if test="${memoList.startPage<=1}">
                        ...
                        <c:if test="${memoList.currentPage>1}">
                            <input type="button" class="button" value="◀" title="이전 페이지로"
                                   onclick="location.href='?currentPage=${memoList.currentPage-1}'">
                        </c:if>
                        <c:if test="${memoList.currentPage<=1}">
                            <input type="button" class="button2" disabled value="◀" title="현재 첫 페이지 입니다.">
                        </c:if>
                        ...
                        <c:forEach var="i" begin="${memoList.startPage}" end="${memoList.endPage}" step="1">
                            <c:if test="${i==memoList.currentPage}">
                                <ruby>
                                    <b>[${i}]</b>
                                    <rt>▼</rt>
                                </ruby>
                        ...
                        <c:if test="${memoList.currentPage<memoList.totalPage}">
                        ...
                        <c:if test="${memoList.endPage<memoList.totalPage}">
                            <input type="button" class="button" value="▶▶" title="다음 10페이지로"
                                   onclick="location.href='?currentPage=${memoList.startPage+10}'">
                        </c:if>
                        <c:if test="${memoList.endPage>=memoList.totalPage}">
                        ...
                        <c:if test="${memoList.currentPage<memoList.totalPage}">
                            <input type="button" class="button" value="▶▶▶" title="마지막 페이지로"
                                   onclick="location.href='?currentPage=${memoList.totalPage}'">
                        </c:if>
                        <c:if test="${memoList.currentPage>=memoList.totalPage}">
                            <input type="button" class="button2" disabled value="▶▶▶" title="현재 마지막 페이지 입니다.">
                        </c:if>
